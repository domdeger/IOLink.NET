...
{
    public static ArrayT Parse(
        XElement elem,
        IParserPartLocator parserLocator,
    ...
    )
    {
        ...
        bool subindexAccessSupported = elem.ReadOptionalAttributeAsBool("subindexAccessSupported");
        DatatypeRefT? typeRef = parserLocator.ParseOptional<DatatypeRefT>(
            elem.Descendants(IODDParserConstants.DatatypeRefName).FirstOrDefault()
        );
        ...
        return new ArrayT(
            ...
            SimpleTypeParser.Parse(
                elem.Descendants(IODDParserConstants.SimpleDatatypeName).FirstOrDefault()
            ),
            typeRef,

...
public class PortInitializer
{
    ...
    public async Task<PortContext> InitializePortAsync(byte port)
    {
        ...
        var (pdInType, pdOutType) = await ResolveProcessDataTypesAsync(port, pdDataResolver);
    ...
    )> ResolveProcessDataTypesAsync(byte port, IProcessDataTypeResolver processDataTypeResolver)
    {
